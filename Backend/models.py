...
caregiver_patient = Table(
    ...
    Column(
        "caregiver_id",
        Integer,
        ForeignKey("caregiver_account.caregiver_id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "patient_id",
        Integer,
        ForeignKey("patient_account.patient_id", ondelete="CASCADE"),
        primary_key=True,
    ),
)
...
class Person(Base):
    ...
    caregiver_account = relationship(
        "CaregiverAccount", back_populates="person", uselist=False
    )
    patient_account = relationship(
        "PatientAccount", back_populates="person", uselist=False
    )
...
class CaregiverAccount(Base):
    ...
    patients = relationship(
        "PatientAccount", secondary=caregiver_patient, back_populates="caregivers"
    )
...
class PatientAccount(Base):
    ...
    caregivers = relationship(
        "CaregiverAccount", secondary=caregiver_patient, back_populates="patients"
    )
    devices = relationship("Device", back_populates="patient", cascade="all, delete")
    relatives = relationship(
        "Relative", back_populates="patient", cascade="all, delete"
    )
...
class Device(Base):
    ...
    measurements = relationship(
        "Measurement", back_populates="device", cascade="all, delete"
    )
...
class Measurement(Base):
    ...
    device_id = Column(
        Integer, ForeignKey("device.device_id", ondelete="CASCADE"), nullable=False
    )
    recorded_at = Column(
        TIMESTAMP(timezone=True), server_default=text("NOW()"), nullable=False
    )
...
class Alert(Base):
    ...
    measurement_id = Column(
        Integer,
        ForeignKey("measurement.measurement_id", ondelete="CASCADE"),
        nullable=False,
    )
    patient_id = Column(
        Integer,
    ...
    acknowledged_by = Column(
        Integer,
        ForeignKey("caregiver_account.caregiver_id", ondelete="SET NULL"),
        nullable=True,
    )
